...
WOptions::WOptions(wxWindow* parent, int id, const wxString& title, const wxPoint& pos, const wxSize& size, long WXUNUSED(style))
    : wxDialog(parent, id, title, pos, size, wxDEFAULT_DIALOG_STYLE)
{
    // begin wxGlade: WOptions::WOptions
...
BEGIN_EVENT_TABLE(WOptions, wxDialog)
    // begin wxGlade: WOptions::event_table
    EVT_CHECKBOX(myID_CHECK_BACKUPS, WOptions::OnCheckboxBackups)
    EVT_CHECKBOX(myID_CHECK_AUTOCLOSE, WOptions::OnCheckboxAutoClose)
    EVT_BUTTON(wxID_OK, WOptions::OnButtonOK)
    // end wxGlade
END_EVENT_TABLE();

void WOptions::OnCheckboxBackups(wxCommandEvent& WXUNUSED(event))
{
...
void WOptions::OnCheckboxAutoClose(wxCommandEvent& WXUNUSED(event))
{
...
void WOptions::OnButtonOK(wxCommandEvent& WXUNUSED(event))
{
    long themeid = listctrlTheme->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
    if (themeid >= 0)
    {
	BitmapCatalog::GetSingleton()->SetTheme(themeid);
    }

    EndModal(wxID_OK);
}

// wxGlade: add WOptions event handlers

void WOptions::set_properties()
{
    // begin wxGlade: WOptions::set_properties
    SetTitle(_("CryptoTE Options"));
    choiceBackupLocation->SetSelection(0);
    // end wxGlade
}

void WOptions::do_layout()
{
    // begin wxGlade: WOptions::do_layout
