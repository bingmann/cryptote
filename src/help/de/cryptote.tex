...
CryptoTE is ein Texteditor mit integrierter, starker Verschlüsselung. Es basiert auf dem bekannten Scintilla editing Widget und verschlüsselt automatisch alle gespeicherte Textdaten in sicheren Containerdateien. Verglichen mit anderen ``Passwortverwaltungs''-Programmen, zwingt CryptoTE dem Benutzer keine Organisation oder Struktur auf: es arbeitet mit einfachem ASCII Text und verlangt von Ihnen keine extra Tabellen, Attribute oder Bezeichnungen auszufüllen.

\section{Über Verschlüsselung}\label{UeberVerschluesselung}

\subsection{Schlechte Passwörter}\label{SchlechtePasswoerter}

Man kann viele Hinweise geben, wie man ein gutes Passwort aussucht. Den Ratschlägen zu folgen, ist aber oft schwer.

Zu Anfang: versuchen sie nicht an ``Pass-Wörter'' zu denken, sondern betrachten sie eher \textbf{Pass-Sätze}. Kein Verschlüsselungsprogramm kann Ihre Daten sichern, wenn Sie das Passwort auf ein einfaches englisches oder deutsches Wort setzen, oder noch schlimmer, ein kurzes Wort, dass mit ihrer Person oder Umgebung im Zusammenhang steht. Im Internet gibt es viele Listen schlechter und dennoch häufig verwendeter Passwörter: zur Erheiterung suchen sie bei Gelegenheit mal nach ``top 100 passwords''.

Mit dem Problem konfrontiert sich ein längeres Passwort merken zu müssen, wollen die meisten Menschen dieses natürlicherweise aufschreiben. Und im nächsten verhängnisvollen Schritt wird dieser Papierschnipsel dann direkt am Rechner aufbewahrt. Machen Sie das besser! 
...
Mein Vorgehen, um ein neues sicheres Passwort zu lernen, ist einfach: ich benutze den Zufallsgenerator, der in CryptoTE integriert ist und lasse diesen ein Zufallszeichenkette generierten. Dieser bietet viele anpassbare Optionen; so verwende ich nie z/y in einem Passwort, da diese auf den deutschen und englischen Tastaturlayouts vertauscht sind. Kein z/y, keine Probleme.
...
\begin{itemize}
  \item Sie vor Ihren Kindern, Mitbewohnern, Kollegen oder Frau zu schützen, die ab und zu den Rechner verwenden: OK.
  \item Um sicher zu gehen, dass keine wichtigen Daten lesbar sind, falls Ihr Notebook oder USB-Stick geklaut wird: auch OK.
  \item Gegen ``Fishing'' oder ``Keylogger'': keine Chance, diese sehen das eingegebene Passwort.
...
\begin{itemize}
  \item Benutzerfreudnliches Scintilla Texteditor-Widget. Dasselbe wie in Notepad++.
  \item Speichert mehrere Text- oder Binärdateien in \helpref{gesicherte private Containern}{Containerdateien}
  \item Hoch-sichere \helpref{Serpent (256 Bits) Verschlüsselung}{Verschluesselung} der sensibelen Daten.
  ...
  \item Schnelle Benutzeroberfläche: Schnell-Suche- und Schnell-Goto-Leisten wie Firefox. Ich benutze das Programm selber täglich.
...
Ein kleiner Fehler im Design des Containerformats kann die gesamte Verschlüsselungskette unsicherer oder sogar knackbar machen. Beachten Sie, dass es recht einfach ist einen solchen Programmierfehler zu machen, und ich bin sicher, dass viele andere ``Passwortschutz''-Programme solche Fehler enthalten.
...
Stattdessen unterstützt der Container mehrere Entschlüsselungspasswörter: \helpref{Mehrere Passwort-Slots}{KeySlots}. Jeder Slot enthält eine verschlüsselte Kopie des Masterschlüssels mit dem die Dateitabelle gelesen werden kann. Die zum Entschlüsseln notwendigen Schlüssel und CBC-IV der Kopien können nur mit dem vom Benutzer eingegeben Passwort bestimmt werden. Aus dem Benutzer-Passwort werden mittels PBKDF2, mit HMAC(SHA256) als Hashfunktion, und zwei verschiedenen zufälligen ``Salts'' der Schlüssel und CBC-IV bestimmt.

Mehr Information zum Containerformat ist in dem CryptoTE Quellcode in libenctain/format.html beigefügt.

Vor dem Entwurf von CryptoTE habe ich eine ausführlichen Geschwindigkeitvergleich verschiedener Kryptographischen Cipher und Bibliotheken gemacht: siehe \urlref{http://idlebox.net}{http://idlebox.net} für mehr Informationen.
...
Der Kompressionsalgorithmus kann in dem Eigenschaften-Dialog der Subdateien eingestellt werden.

CryptoTE enthält auch bzip2 als alternative Kompressionsmethode. Diese ist generell jedoch nur für wirklich große Textdateien besser, für kleine Texte ist zlib die besser Wahl.
...
Für Unix-User und andere fortgeschrittene Anwender enthält CryptoTE auch ein Kommandozeileninterface. Dies ist sehr nützlich, wenn man keine graphische Oberfläche starten kann, z. B. falls man mit ssh eingeloggt ist oder X11 gerade nicht zur Verfügung hat.

Der einfachste Weg CryptoTE ohne graphische Oberfläche zu starten ist: \texttt{cryptote -s file.ect}.

Dies fragt zuerst das Containerpasswort ab und startet bei gelungener Entschlüsselung eine einfache Eingabeoberfläche. Mit ``help'' erhalten Sie dann eine Liste verfügbarer Befehle unter weitere Informationen.
