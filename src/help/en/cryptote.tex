...
Much advice can be given about choosing a good password. Following the advice is often more difficult.

First: do not think of ``passwords'', rather think of \textbf{pass-phrases}. No encryption program can keep your data safe if you set the password to a plain English word or, even worse, some word connected with your surroundings or identity. There are many lists of bad but nevertheless frequently used passwords on the Internet: try search for ``top 100 passwords'' for some amusement.
...
\begin{itemize}
  \item To keep them safe against your children, roommates, co-workers or wife casually using your computer: OK.
...
\begin{itemize}
  ...
  \item Also useable from the Unix/Linux \helpref{command line}{Commandline} on a text console e.g. via ssh.
...
The container file format support \helpref{built-in encryption}{Encryption} and \helpref{built-in compression}{Compression}. It also supports multiple key slots for different password.
